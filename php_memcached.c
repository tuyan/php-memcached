...
static PHP_METHOD(Memcached, getResultString)
{
	...
	switch (i_obj->rescode) {
		...
		default:
			RETURN_STRING(memcached_strerror(i_obj->memc, i_obj->rescode), 1);
	}
}
/* }}} */
...
static int php_memc_handle_error(php_memc_t *memc_obj, memcached_return status TSRMLS_DC)
{
	int result = 0;
